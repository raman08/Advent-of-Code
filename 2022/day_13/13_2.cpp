...
int main(int argc, char* argv[]) {
    ...
    vector<string> divider = {"[[2]]", "[[6]]"};

    packets.push_back(divider[0]);
    packets.push_back(divider[1]);

    sort(packets.begin(), packets.end(),
         [&](string& s1, string& s2) { return compare(s1, s2) == -1; });

    int i1, i2;
    for (int i = 0; i < packets.size(); i++) {
        cout << packets[i] << endl;
        if (packets[i] == divider[0]) { i1 = i + 1; }
        if (packets[i] == divider[1]) { i2 = i + 1; }
    }
