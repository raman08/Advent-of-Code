...
long long findResultRev(long long op1, long long op2, char op) {
    if (op == '-') { return op1 + op2; }
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        for (auto& monkey : monkeys) {
            ...
            if (!monkey.calculated && monkey.name != "humn") {
            ...
            if (monkey.calculated && monkey.type == SOLVE) {
                int m1 = monkeysMap[monkey.dep1];
                int m2 = monkeysMap[monkey.dep2];

                if (monkeys[m1].calculated &&
                    monkeys[m2].calculated) {
                    continue;

                } else if (monkeys[m1].calculated &&
                           !monkeys[m2].calculated) {
                    monkeys[m2].val = findResultRev(
                        monkey.val, monkeys[m1].val,
                        monkey.operation);

                    if (monkey.operation == '/') {
                        monkeys[m2].val =
                            monkeys[m1].val / monkey.val;
                    }
                    ...
                    monkeys[m2].calculated = true;

                } else if (!monkeys[m1].calculated &&
                           monkeys[m2].calculated) {
                    monkeys[m1].val = findResultRev(
                        monkey.val, monkeys[m2].val,
                        monkey.operation);

                    monkeys[m1].calculated = true;
                } else if (!monkeys[m1].calculated &&
                           !monkeys[m2].calculated) {
                    continue;
                }

                if (monkeys[monkeysMap["humn"]].calculated) {
                    cout << monkeys[monkeysMap["humn"]].val
                         << endl;
                    return 0;
                }
            }
