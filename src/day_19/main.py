    ...
    def analyze_workflow(self, pr: PartRange):
        ...
        for this_flow in self.flows:
            if "<" in this_flow or ">" in this_flow:  # Example: a<2006:qkq
                ...
                compare_to = int(re.findall("\d+", this_flow)
                                 [0])  # example is 2006
                # result = qkq for the example
                _, result = this_flow.split(":")
                # For the example, gives the range for 'a' category
                cat_range = pr.cat(category)
                if "<" in this_flow:
                    if (
                        len(range(cat_range.start, compare_to)) > 0
                        ...
                        new_range.replace(category, range(
                            cat_range.start, compare_to))
                    ...
                    if len(range(compare_to, cat_range.stop)) > 0:
                        ...
                        new_range.replace(
                            category, range(compare_to + 1, cat_range.stop)
                        )
                        splits.append([result, new_range])

                        pr.replace(category, range(
                            cat_range.start, compare_to + 1))
            else:
...
# Part 2
start_range = PartRange(range(1, 4001), range(1, 4001),
